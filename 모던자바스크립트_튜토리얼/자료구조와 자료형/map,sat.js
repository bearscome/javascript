...
// map set 다시 공부
/*
    맵은 키에 불린, 숫자, 문자열로 저장 가능
    map.set, map.get 가능
    객체와의 차이점 이라면 맵은 더욱 다양하게 활용 가능 객체는 키 값은 무조건 문자형으로 변환 됨

    set 는 중복값이 들어 갈 수 없음
    let set = [1,2,3,4,1,2,3]
    을 하면 1,2,3,4 만 나옴

    그래서 중복 제거할 때 유용
    셋을 다시 배열로 바꾸면 됨
    Array.from(), forEach(), 등을 이용하면 다시 배열로 바꿀 수 있음
 */